...
HEAVY_EXPLAIN_CONCEPTS = {"dominance", "double_top", "double_bottom", "neckline", "bull_flag", "bear_flag"}
...
def build_alert(symbol, candle, alert, ema_21, ema_55, current_rsi, volume_avg, skill_level=None):
    ...
    if alert.get("type", "").endswith(":weak_break"):
        ...
        return (
            ...
            f"<b>SETUP GRADE:</b> {trade_plan['setup_quality']}\n"
            ...
            f"<b>Why it didn't confirm:</b> Break detected, but momentum/volume did not confirm.\n"
            f"Nothing to do here. This one hasn't earned a look yet.\n\n"
            f"{link_text.lstrip()}"
        )
    ...
    if alert.get("type", "").endswith(":failed_follow_through"):
        ...
        return (
            ...
            f"{secondary_text}"
            f"<b>Next watch zone:</b> around {format_level(location.get('next_target', close))}\n\n"
            f"<b>Why it didn't confirm:</b> Price stalled around the broken zone with weak volume.\n"
            f"Nothing to do here. This one hasn't earned a look yet.\n\n"
            f"{link_text.lstrip()}"
        )
    ...
    if alert.get("type", "").endswith(":late_move"):
        ...
        return (
            ...
            f"<b>Close:</b> {format_level(close)}\n"
            f"<b>SETUP GRADE:</b> {alert.get('setup_quality', 'D')}\n"
    ...
    if trade_plan:
        ...
        if location:
            location_text = (
                ...
                f"<b>Range low:</b> {format_level(location['range_low'])}\n"
                f"<b>Range high:</b> {format_level(location['range_high'])}\n\n"
                f"<b>Room to next zone:</b> {location['room_to_target']}\n"
                f"<b>Location Quality:</b> {location['location_quality']}\n"
                f"<b>Next zone:</b> {format_level(location['next_target'])}\n"
                f"<b>Distance to next zone:</b> "
        ...
        return (
            ...
            f"<b>WHAT THE INDICATORS SAY</b> (they reinforce — price is the truth)\n"
            ...
            f"{secondary_text}"
            f"{location_text}"
            f"<b>WHAT WOULD BREAK THIS READ:</b>\n"
            f"A daily close back {'above' if trade_plan['direction'] == 'SHORT' else 'below'} "
            f"{format_level(trade_plan['level'])} would reclaim the zone and "
            f"invalidate this break.\n\n"
            f"This is a heads-up, not a call. You're at the place you said you'd be "
            f"watching. Go look — you already decided what to do here."
            f"{link_text}"
        )
    ...
    if alert.get("setup_quality"):
        setup_text = (
            f"⭐ <b>SETUP GRADE:</b> {alert['setup_quality']}\n"
...
def poinkle_onboarding_text(kind):
    ...
    if kind == "help":
        return (
            "Poinkle watches the zones you care about and tells you when one actually "
            "breaks — confirmed by two daily closes, not one.\n\n"
            "It never tells you what to do. No entries, no stops, no targets, no calls. "
